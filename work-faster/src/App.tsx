...
function App() {
  ...
  const [startImage, setStartImage] = useState(playImg);
  ...
  useEffect(() => {
    if (timeLeft === 0 && isRunning) {
      ...
      setStartImage(playImg); // Reset to play button
  ...
  const handleClick = () => {
    if (!isRunning) {
      ...
      setStartImage(resetImg);
    } else {
      setIsRunning(false);
      setTimeLeft(isBreak ? 5 * 60 : 25 * 60);
      setGifImage(idleGif);
      setStartImage(playImg);
    }
  };
    ...
    <div className={containerClass} style={{ position: "relative" }}>
      ...
      <div className="home-content">
        ...
        <button className="home-button" onClick={handleClick}>
          <img src={startImage} alt="Button Icon" />
        </button>
      </div>
    </div>
  );
